...
class CharRulesItems(CharRules):
    ...
    def goro_available(self, state: CollectionState, player: int) -> bool:
        return (super().goro_available(state, player) and
                state.has_all_counts({"Fluffy Doughnut": 1, "Fish Candy": 1, "Fruit of Eden": 2,
                                      "Pocket": 1}, player))

    def ruby_available(self, state: CollectionState, player: int) -> bool:
        return (super().ruby_available(state, player) and
                state.has_all_counts({"Fluffy Doughnut": 2, "Fish Candy": 2, "Grass Cake": 1,
                                      "Fruit of Eden": 6, "Pocket": 2}, player))

    def ungaga_available(self, state: CollectionState, player: int) -> bool:
        return (super().ungaga_available(state, player) and
                state.has_all_counts({"Fluffy Doughnut": 3, "Fish Candy": 3, "Grass Cake": 2,
                                      "Witch Parfait": 1, "Fruit of Eden": 10, "Pocket": 3}, player))

    def osmond_available(self, state: CollectionState, player: int) -> bool:
        return (super().osmond_available(state, player) and
                state.has_all_counts({"Fluffy Doughnut": 4, "Fish Candy": 4, "Grass Cake": 3,
